...
def start_command_handler_New(update, context):
    """Send a message when the command /start is issued."""
    add_typing(update, context)
    

    user_input = get_text_from_message(update)
    q0=user_input.split('1)')
    qution=q0[0]
    q1=user_input.split('1)')[1]
    op1=q1.split('2)')
    q2=q1.split('2)')[1]
    op2=q2.split('3)')
    q3=q2.split('3)')[1]
    ...
    op4=q4.split('5)')
    op5=(q4.split('5)'))
    s=[len(q0),len(op1),len(op2),len(op3),len(op4)]
    logging.info(f"user_input : {s}")
    z=s.index(3)

    if(len(qution)<=300):
        quiz_question = QuizQuestion()
        quiz_question.question = qution
        quiz_question.answers = [op1[0],op2[0],op3[0],op4[0],op5[1].replace('Answer','')]
        quiz_question.correct_answer_position = z
        quiz_question.correct_answer = "wine"

        add_quiz_question(update, context, quiz_question)
    else:
        add_text_message(update, context, qution)
        quiz_question = QuizQuestion()
        quiz_question.question = "Find Correct Answer"
        quiz_question.answers = [op1[0],op2[0],op3[0],op4[0],op5[1].replace('Answer','')]
        quiz_question.correct_answer_position = z
        quiz_question.correct_answer = "wine"
...
def main_handler(update, context):
    ...
    if update.message is not None:
        ...
        # logging.info(f"user_input : {user_input1}")
        # logging.info(f"user_input : {user_input2}")
        # logging.info(f"user_input : {user_input3}")
        # logging.info(f"user_input : {user_input4}")
        # logging.info(f"user_input : {user_input5}")
        # logging.info(f"user_input : {user_input6}")
...
def main():
    ...
    # message handler
    dp.add_handler(MessageHandler(Filters.text, start_command_handler_New))
...
class DefaultConfig:
    ...
    TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "5050841485:AAEM4AxP13kDwp39nYOSTilZXm8Id26GiEg")
    MODE = os.environ.get("MODE", "polling")
    WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "https://script.google.com/macros/s/AKfycbzmfSewA9slhcPKi3MAxdMHab_Leej0H1DHMLygAu2QiSfUyMLjdBr0fFA7ZviyHngPlQ/exec")
